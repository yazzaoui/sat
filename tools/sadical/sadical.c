...
static void assign (Solver * solver, int lit, Clause * reason) {
  ...
  if (!solver->level) solver->local.units++, reason = 0;
  int sign = lit < 0 ? -1 : 1;
...
// Witness event log emission (JSONL, see 'docs/event-log-schema.md').

static void emit_attempt_event (Solver * solver, long id,
                                long considered, long filtered,
                                long copied, long assumed) {
  SaDiCaL * sadical = solver->sadical;
  FILE * f = sadical->eventlog;
  fprintf (f,
    "{\"ev\":\"attempt\",\"id\":%ld,\"conflicts\":%ld,\"decisions\":%ld,"
    "\"level\":%d,\"trail_size\":%ld,\"decisions_lits\":[",
    id, solver->local.conflicts, solver->local.decisions,
    solver->level, (long) SIZE (solver->trail));
  for (int level = 1; level <= solver->level; level++)
    fprintf (f, "%s%d", level == 1 ? "" : ",",
      solver->frames.begin[level].decision);
  fprintf (f,
    "],\"reduct\":{\"considered\":%ld,\"filtered\":%ld,"
    "\"copied\":%ld,\"assumed\":%ld},\"trail\":[",
...
static void emit_accept_event (Solver * solver, long id,
                               double ms, long conflicts, long decisions) {
  SaDiCaL * sadical = solver->sadical;
  FILE * f = sadical->eventlog;
  Solver * inner = sadical->inner;

  // Same flipped-decision search and witness mapping as in
  // 'trace_pruned_clause' (keep the two in sync).

  int flipped = 0;
  for (int level = solver->level; !flipped && level > 0; level--) {
    int decision = solver->frames.begin[level].decision;
    if (val (inner, map (solver, decision)) < 0) flipped = decision;
  }
  fprintf (f, "{\"ev\":\"accept\",\"id\":%ld,\"clause\":[%d", id, -flipped);
  for (int level = solver->level; level > 0; level--) {
    int decision = solver->frames.begin[level].decision;
    if (decision != flipped) fprintf (f, ",%d", -decision);
  }
  fprintf (f, "],\"witness\":[%d", -flipped);
  for (int * p = solver->trail.begin; p < solver->trail.end; p++) {
    int outer_lit = *p;
    if (outer_lit == flipped) continue;
    Var * v = var (solver, outer_lit);
    if (v->stamp < solver->stamp) continue;	// Not mapped.
    int tmp = val (inner, map (solver, outer_lit));
    fprintf (f, ",%d", tmp < 0 ? -outer_lit : outer_lit);
  }
  fputs ("],\"flipped\":[", f);
  bool first = true;
  for (int * p = solver->trail.begin; p < solver->trail.end; p++) {
    int outer_lit = *p;
    if (val (inner, map (solver, outer_lit)) >= 0) continue;
    fprintf (f, "%s%d", first ? "" : ",", outer_lit);
    first = false;
  }
  fprintf (f,
...
static void emit_reject_event (Solver * solver, long id,
                               double ms, long conflicts, long decisions) {
  fprintf (solver->sadical->eventlog,
    "{\"ev\":\"reject\",\"id\":%ld,"
    "\"inner\":{\"time_ms\":%.3f,\"conflicts\":%ld,\"decisions\":%ld}}\n",
    id, ms, conflicts, decisions);
}
...
static bool prune (Solver * solver) {
  ...
  long ev_considered = sadical->reduct.clauses.considered;
  long ev_filtered = sadical->reduct.clauses.filtered;
  ...
  double ev_start = 0;
  if (sadical->eventlog) {
    ev_id = ++sadical->attempts;
    emit_attempt_event (solver, ev_id,
      sadical->reduct.clauses.considered - ev_considered,
      sadical->reduct.clauses.filtered - ev_filtered,
      sadical->reduct.clauses.copied - ev_copied,
      sadical->reduct.filter.assumed - ev_assumed);
    ev_start = sadical_process_time ();
  ...
  if (witness_found) {
    ...
    if (sadical->eventlog)
      emit_accept_event (solver, ev_id,
        (sadical_process_time () - ev_start) * 1e3,
        sadical->inner->local.conflicts - ev_conflicts,
    ...
    if (sadical->eventlog)
      emit_reject_event (solver, ev_id,
        (sadical_process_time () - ev_start) * 1e3,
        sadical->inner->local.conflicts - ev_conflicts,
        sadical->inner->local.decisions - ev_decisions);
    res = false;
...
int sadical_solve (SaDiCaL * sadical) {
  if (sadical->eventlog)
    fprintf (sadical->eventlog,
      "{\"ev\":\"run_start\",\"schema\":1,\"cnf\":\"%s\",\"vars\":%d,"
      "\"clauses\":%ld,\"options\":{\"filter\":%d,\"nonroot\":%d,"
      "\"level\":%d,\"prune\":%d}}\n",
      sadical->input_path, sadical->outer->max_var,
      (long) SIZE (sadical->outer->clauses),
      (int) sadical->opts.filter, sadical->opts.nonroot,
      sadical->opts.level, (int) sadical->opts.prune);
  double ev_start = sadical_process_time ();
